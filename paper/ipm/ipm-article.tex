...
Algorithm~\ref{alg:soft-o2} records that full stage order; $\beta$ acts on the score field present at each expansion stage in \texttt{memory\_manager.py}.
...
The primary hypothesis family is Soft~O2 vs FlatIP on the nine CAIL and LegalEp transfer channels; for that family we report McNemar mid-$p$ on paired per-query Answer Hit and Holm-adjusted $p$-values (Table~\ref{tab:sig}), computed from the V4 paired-hit archive used for significance testing.
...
Holm $\Delta$AH values in Table~\ref{tab:sig} may differ slightly from rebuild point estimates because they use the paired V4 hit archive; directional Soft~O2$>$FlatIP conclusions on non-exact channels agree across both sources.
...
\caption{CAIL2024 multi-turn results at tier $M\approx 3000$. Soft~O2 improves AH and EC over FlatIP; shuffled $sid$ removes the gain; hard expansion can further raise AH/EC as a score-copy envelope.}
\label{fig:cail_main}
\end{figure}
...
Hybrid uses the post-gate direct-dense trigger: cluster binding fires only on tids recorded \emph{before} Soft~O2 session expansion.
...
\end{tabular}
\end{table}
{\footnotesize Hybrid uses the post-gate direct-dense trigger. Soft~O2-C mid-$p$ vs Soft~O2 may favour either side; on LegalMem-MT the ${}^{**}$ marks Soft~O2-C significantly \emph{below} Soft~O2. Cluster co-membership of Sa/Sb pairs follows Section~\ref{sec:csce-protocol}.}
...
After Holm adjustment on the Soft~O2 vs FlatIP family (Table~\ref{tab:sig}; V4 paired hits), the non-exact CAIL and LegalEp gains remain significant.
...
On CAIL multi-turn and LegalEp paraphrase~/advice-recall channels, Soft~O2 improves Answer Hit and graded nDCG over FlatIP and cuts incompleteness; hard hydration can further raise AH/EC as a score-copy envelope, while shuffled-$sid$ controls collapse Soft~O2 gains.
...
\end{tabular}
\end{table}
{\footnotesize Soft~O2 improves AH/EC over FlatIP; hard hydration is the score-copy envelope and can lead both.}
