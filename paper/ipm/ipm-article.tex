...
We report graded nDCG alongside Answer Hit and Episode Completeness, with paired significance tests and a failure taxonomy separating complete, incomplete, and missed-session retrieval.
\end{abstract}
...
Answer Hit and Episode Completeness are the primary Soft~O2 claims; nDCG uses the fixed gold-session IDCG above so that systems remain comparable on each query.
...
When gold answers already share $sid$ with the query, Soft~O2 is the natural operator and cluster binding is unnecessary. The complementary question is whether Soft~O2-C helps when evidence is deliberately placed off-session. Table~\ref{tab:csce} uses the Mix protocol of Section~\ref{sec:csce-protocol}: $70\%$ cross-session Sa/Sb gold and $30\%$ intact same-session gold, with FlatIP, Soft~O2, Soft~O2-C, and gated Hybrid sharing unrestricted dense retrieval. On the cross-session stratum, Soft~O2-C improves over Soft~O2 on LegalEp-Lawyer ($0.491$ vs $0.446$) and LegalMem-MT ($0.537$ vs $0.503$), showing that cluster soft binding can recover split-episode gold that session membership alone cannot surface. Gated Hybrid Soft~O2+Soft~O2-C turns this mechanism into an overall gain: LegalEp-Lawyer AH rises to $0.534$ from Soft~O2 $0.496$ (mid-$p{=}0.010$${}^{*}$), and LegalMem-MT reaches $0.646$ vs Soft~O2 $0.618$, while most same-session hits are preserved ($0.560$~/~$0.867$). Ungated Soft~O2-C can raise cross-session hits yet displace intact same-session evidence, which is why Hybrid triggers cluster binding only on direct dense hits. Same-session Soft~O2 vs Soft~O2-C contrasts remain Soft~O2-led and are reported in Appendix~\ref{app:tables} (Table~\ref{tab:cluster_same}).
...
RQ3--RQ4 follow Soft~O2 $\rightarrow$ Soft~O2-C: under Mix, gated Hybrid recovers additional cross-session gold (Table~\ref{tab:csce}), while Soft~O2 remains the default when gold already shares $sid$ (Table~\ref{tab:cluster_same}).
