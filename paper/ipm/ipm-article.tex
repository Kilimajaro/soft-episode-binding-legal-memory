...
% Break long checkpoint ids after hyphens or slashes (avoids margin overflow).
\DeclareRobustCommand{\modelid}[1]{{\ttfamily\modelidbreak#1\relax}}
\def\modelidbreak#1{%
  \ifx#1\relax\else
    \ifx#1--\hspace{0pt}\else
    \ifx#1//\hspace{0pt}\else#1\fi\fi
    \expandafter\modelidbreak
  \fi}
\newcommand{\tbd}{---}
...
\newcommand{\tighttab}{\setlength{\tabcolsep}{3.0pt}\renewcommand{\arraystretch}{1.05}}
...
The design target is episode completeness under same-domain interference: Soft~O2 binds session siblings by attenuated score inheritance ($\beta s$) rather than hard score copy, so a retrieved question can surface its advice turn without dumping an entire session into top-$k$.
Exact replay is a surface-form reference; main channels are paraphrase, follow-up, and advice-recall.
On a shared turn-level FlatIP rebuild, Soft~O2 raises Answer Hit over dense FlatIP; hard hydration, which copies sibling scores without attenuation, can raise AH further by forcing siblings into top-$k$.
Soft~O2-C is evaluated as a conditional cluster-binding mechanism under Mix reconstructions with off-session gold; gated Hybrid is the composed operator when thematic co-membership helps.
We report Answer Hit, Episode Completeness, graded nDCG with fixed gold-session IDCG, a four-way failure taxonomy, and Holm-adjusted Soft~O2 vs FlatIP tests that reuse the same per-query Answer Hit outcomes as the Soft~O2 tables.
\end{abstract}
...
Soft~O2 therefore completes missing siblings by attenuated score inheritance rather than hard score copy.

This paper presents BIMS-LEGAL, a dual-store system for Chinese legal consultation logs.
A fast episodic pathway indexes timestamped turn embeddings with speaker role and session identifier $sid$; a slower semantic pathway consolidates turns with target-$k$ KMeans.
We adopt a dual-pathway layout---fast episode index; slower semantic consolidation---as an architectural analogy to Complementary Learning Systems \cite{mcclelland1995complementary, kumaran2016}, without claiming a neuroscientific equivalence.
...
We evaluate Soft~O2 primarily against FlatIP under episode incompleteness, treat hard hydration as an unrestricted score-copy upper bound, and examine Soft~O2-C under Mix reconstructions as a conditional mechanism test.
...
Second, we propose Soft~O2 as a turn-level soft-binding operator: a gap-ratio score-competition analysis identifies a feasible high-$\beta$ band $\rho{<}\beta{<}1$, and an Answer Hit sensitivity sweep over $\{0.5,0.7,0.9,0.95,0.98,1.0\}$ confirms a high-AH plateau from which we freeze default $\beta{=}0.98$ (Table~\ref{tab:beta}), with O1/O3 as implementation operators for shared-store evaluation.
Third, we evaluate Soft~O2 on CAIL multi-turn and rebuilt LegalEp corpora against FlatIP, hard hydration, and shuffled-$sid$ under a shared FlatIP rebuild, with BM25-turn, RRF, and cross-encoder controls as secondary comparisons, and report Soft~O2-C Mix contrasts as conditional mechanism evidence with graded nDCG, a four-way failure taxonomy, and Holm-adjusted Soft~O2 vs FlatIP tests.
...
Legal consultation archives compound that difficulty: high lexical overlap among topically adjacent matters, with the professionally useful evidence unit being the full question--answer episode.
...
BIMS-LEGAL indexes episodic turns with $sid$ (Soft~O2) and thematic KMeans clusters (Soft~O2-C) when query and gold evidence occupy different sessions but share thematic structure.
...
Algorithm~\ref{alg:soft-o2} records that full stage order; $\beta$ acts on the score field present at each expansion stage in the released memory manager.

\begin{algorithm}[t]
\caption{Soft~O2 session soft binding (implementation order)}
\label{alg:soft-o2}
\begin{algorithmic}[1]
\Require Query $q$, dense/associative hits $\mathcal{R}_0$, top-$k$, attenuation $\beta$
\State $\mathcal{R}\leftarrow\mathsf{ExpandSiblings}(\mathcal{R}_0;\beta)$ \Comment{raw-score Soft~O2}
\State $\mathcal{R}\leftarrow\mathsf{ExactBoost}/\mathsf{SparseAugment}(q,\mathcal{R})$ \Comment{optional}
\State $\mathcal{R}\leftarrow\mathsf{SessionFirstRerank}(\mathcal{R},k)$
\State fuse each $r\in\mathcal{R}$ with Eq.~\eqref{eq:base-score}
\State $\mathcal{R}\leftarrow\mathsf{EnsureSessionCompleteness}(\mathcal{R};\beta)$ \Comment{final Soft~O2 pass}
\State \Return top-$k$ of $\mathcal{R}$ by descending fused score
\end{algorithmic}
\end{algorithm}
...
Table~\ref{tab:beta-theory} summarizes the empirical gap-ratio distribution $\rho=s_d/s_h$ on LegalEp stores under an exact-channel query proxy: median $\rho$ lies well below one, so a high-$\beta$ point near $0.98$ covers most measured distractors while remaining strictly below hard hydration ($\beta{=}1$).
A three-candidate score-competition model yields the feasible band $\rho{<}\beta{<}1$; we then confirm the operating region with an Answer Hit sensitivity sweep rather than treating a closed-form hinge as a selector (a zero-margin hinge on $\beta\in(0,1]$ is uninformative because its rank term vanishes for all $\beta<1$).

\begin{table}[t]
\centering
\caption{Gap-ratio statistics on LegalEp $M{=}3000$ stores (exact-channel query proxy; strongest measured non-session distractor). The feasible band $\rho{<}\beta{<}1$ motivates the high-$\beta$ operating region later confirmed by the AH grid search in Table~\ref{tab:beta}.}
...
\paragraph{Soft-rank visualization and AH sensitivity sweep.}
As a theoretical diagnostic we also plot the soft pairwise surrogate
...
$\mathcal{L}_{\mathrm{sr}}$ visualizes the availability--ranking trade-off under the measured $\rho$ pool; it is not the selector.
Default $\beta{=}0.98$ is frozen after an Answer Hit sensitivity sweep over $\{0.5,0.7,0.9,0.95,0.98,1.0\}$ on the CAIL Uk-followup and LegalEp exact/paraphrase evaluation channels (Table~\ref{tab:beta}).
The high-AH plateau $\{0.9,0.95,0.98\}$ coincides with $\rho{<}\beta{<}1$; $\beta{=}1$ is not better and $\beta{=}0.5$ collapses binding.
The sweep uses the same evaluation query pools as the Soft~O2 grids---\emph{not} a held-out validation split---and comes from LegalMem-MT $\beta$-campaign stores.
Absolute AH in Table~\ref{tab:beta} is therefore not comparable to the unified FlatIP rebuild (Tables~\ref{tab:soft-o2-controls}--\ref{tab:lawyer_main}), which freezes $\beta{=}0.98$ after selection; grid provenance (campaign ids, pool sizes, overlap flag) ships with the manuscript package.
Advice-recall inherits the same frozen default without a separate gap-ratio fit.
...
Semantic consolidation uses target-$k$ KMeans over turn embeddings, producing cluster ids consumed by Soft~O2-C.
Other agglomerative helpers that may appear in the repository are unused on the Soft~O2-C evaluation path.
...
The primary hypothesis family is Soft~O2 vs FlatIP on the nine CAIL and LegalEp transfer channels.
AH, EC, nDCG, failure rates, and Table~\ref{tab:sig} (McNemar mid-$p$ with Holm adjustment) share one FlatIP rebuild and the same per-query Answer Hit outcomes; each $\Delta$AH in Table~\ref{tab:sig} equals Soft~O2$-$FlatIP in the Soft~O2 tables up to ordinary rounding.
Appendix Soft~O2 grids report AH/EC/nDCG point estimates from that rebuild without separate CI columns.
...
A cross-encoder (CE) control reranks the top-$30$ FlatIP candidates with \modelid{BAAI/bge-reranker-v2-m3}.
...
FlatIP, Soft~O2, Soft~O2-C, and gated Hybrid share unrestricted dense retrieval on this store; we report overall AH and stratum AH on cross-session vs same-session queries.
...
Unless noted, all dense retrieval grids use the multilingual encoder
\modelid{nomic-embed-text-v2-moe}~\cite{nussbaum2025nomic} ($768$-d)
as a single shared embedding backend, with $k{=}10$ and seed~$42$ on the main $M{\approx}3000$ stores.
The encoder is held fixed so Soft~O2 vs FlatIP contrasts isolate binding rather than representation changes; sensitivity to alternative Chinese or multilingual encoders (e.g., \modelid{bge-large-zh-v1.5}) is discussed in Section~\ref{sec:limitations}.
...
Table~\ref{tab:o1o2} reports the shared-store ablation on the DISC-Law and
Lawyer\-LLaMA corpora ($M{=}400$, $S{=}300$).
IVFPQ baseline Answer Hit is $0.540$ on DISC and $0.397$ on Lawyer.
O1 alone yields a small gain on DISC ($0.567$) and a near-tie on Lawyer ($0.397$).
Exact indexing is therefore a necessary prerequisite, but it is insufficient alone.
O1+Soft~O2 raises Answer Hit to $0.780$ (DISC) and $0.680$ (Lawyer), gains of $+0.240$ and $+0.283$ over baseline, with Episode Completeness $0.888$ and $0.840$.
...
Hard hydration can drive incompleteness near zero by score copy and often leads Soft~O2 on complete rate (Section~\ref{sec:exp-main}).

\begin{table}[t]
\centering
\caption{Failure taxonomy at $k{=}10$ (values in \%; rows sum to $100$ within rounding). DISC/Lawyer denote LegalEp corpora. Classes: complete (Q+A), incomplete (Q hit~/~A miss), answer-only, and session miss. Boldface: best complete~/~lowest incomplete among FlatIP, Soft~O2, and shuffled~$sid$; hard hydration is unrestricted score copy.}
\label{tab:rq1-failure}
\tighttab\footnotesize
\begin{tabular}{@{}llcccc@{}}
\toprule
\textbf{Channel} & \textbf{System} & \textbf{Comp.} & \textbf{Inc.} & \textbf{A-only} & \textbf{Miss} \\
\midrule
DISC / u-para & FlatIP & 68.8 & 28.8 & 0.8 & 1.6 \\
DISC / u-para & Soft O2 & \textbf{88.1} & \textbf{9.9} & 0.2 & 1.8 \\
DISC / u-para & Hard & 97.2 & 0.0 & 0.0 & 2.8 \\
DISC / u-para & Shuf.\ O2 & 61.6 & 35.8 & 0.4 & 2.2 \\
\midrule
DISC / advice & FlatIP & 47.2 & 17.6 & 0.0 & 35.2 \\
DISC / advice & Soft O2 & \textbf{61.0} & \textbf{3.6} & 0.0 & 35.4 \\
DISC / advice & Hard & 64.8 & 0.0 & 0.0 & 35.2 \\
DISC / advice & Shuf.\ O2 & 43.4 & 21.2 & 0.0 & 35.4 \\
\midrule
Lawyer / u-para & FlatIP & 66.9 & 29.5 & 2.8 & 0.8 \\
Lawyer / u-para & Soft O2 & \textbf{94.6} & \textbf{2.8} & 1.0 & 1.6 \\
Lawyer / u-para & Hard & 97.8 & 0.0 & 0.0 & 2.2 \\
Lawyer / u-para & Shuf.\ O2 & 64.9 & 30.7 & 2.6 & 1.8 \\
\midrule
CAIL / Uk & FlatIP & 43.2 & 56.7 & 0.0 & 0.2 \\
CAIL / Uk & Soft O2 & \textbf{93.2} & \textbf{6.7} & 0.0 & 0.2 \\
CAIL / Uk & Hard & 99.7 & 0.2 & 0.0 & 0.2 \\
CAIL / Uk & Shuf.\ O2 & 31.3 & 68.5 & 0.0 & 0.2 \\
\bottomrule
\end{tabular}
\end{table}
...
Hard hydration---full sibling score copy---can exceed Soft~O2 AH on the same rebuild (Table~\ref{tab:soft-o2-controls}); Soft~O2 injects siblings at attenuated scores $\beta s$ with $\beta{<}1$ (Tables~\ref{tab:cail_main}--\ref{tab:lawyer_main}).
...
\begin{table}[t]
\centering
\caption{Primary Soft~O2 controls (AH@10, $M{\approx}3000$, unified FlatIP rebuild). Boldface: best AH within each row among FlatIP / Soft~O2 / Hard / Shuffled. Soft~O2 is compared primarily to FlatIP; hard hydration is unrestricted score copy; shuffled~$sid$ tests membership dependence.}
...
\midrule
CAIL / U1 & 0.803 & 0.948 & \textbf{0.995} & 0.660 \\
CAIL / Uk & 0.432 & 0.932 & \textbf{0.997} & 0.313 \\
CAIL / U-last & 0.393 & 0.957 & \textbf{0.998} & 0.255 \\
LegalEp-DISC / u-para & 0.696 & 0.883 & \textbf{0.972} & 0.620 \\
LegalEp-DISC / advice & 0.472 & 0.610 & \textbf{0.648} & 0.434 \\
LegalEp-Lawyer / u-para & 0.697 & 0.956 & \textbf{0.978} & 0.675 \\
LegalEp-Lawyer / advice & 0.416 & 0.654 & \textbf{0.674} & 0.396 \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
CAIL & Uk & FlatIP & 0.428 & 56.7\% \\
CAIL & Uk & Soft O2 & \textbf{0.796} & 6.7\% \\
CAIL & Uk & Shuffled O2 & 0.349 & 68.5\% \\
CAIL & U-last & FlatIP & 0.400 & 60.5\% \\
CAIL & U-last & Soft O2 & \textbf{0.796} & 4.2\% \\
CAIL & U-last & Shuffled O2 & 0.326 & 74.3\% \\
LegalEp-DISC & u-para & FlatIP & 0.659 & 28.8\% \\
LegalEp-DISC & u-para & Soft O2 & \textbf{0.742} & 9.9\% \\
LegalEp-DISC & u-para & Shuffled O2 & 0.590 & 35.8\% \\
LegalEp-DISC & advice & FlatIP & 0.454 & 17.6\% \\
LegalEp-DISC & advice & Soft O2 & \textbf{0.518} & 3.6\% \\
LegalEp-DISC & advice & Shuffled O2 & 0.411 & 21.2\% \\
LegalEp-Lawyer & u-para & FlatIP & 0.708 & 29.5\% \\
LegalEp-Lawyer & u-para & Soft O2 & \textbf{0.832} & 2.8\% \\
LegalEp-Lawyer & u-para & Shuffled O2 & 0.656 & 30.7\% \\
LegalEp-Lawyer & advice & FlatIP & 0.457 & 26.2\% \\
LegalEp-Lawyer & advice & Soft O2 & \textbf{0.569} & 2.0\% \\
LegalEp-Lawyer & advice & Shuffled O2 & 0.424 & 27.8\% \\
\bottomrule
\end{tabular}
\end{table}
...
Soft~O2 raises Answer Hit over FlatIP on every channel (U1 $0.948$ vs $0.803$; Uk $0.932$ vs $0.432$; U-last $0.957$ vs $0.393$).
Shuffled $sid$ removes the gain, tying the improvement to correct episode membership.
Hard hydration can raise AH further by copying sibling scores without attenuation (e.g., Uk $0.997$ vs Soft~O2 $0.932$).
Table~\ref{tab:ndcg-graded} preserves the Soft~O2$>$FlatIP ordering under graded nDCG with gold-session IDCG, and incompleteness falls sharply (Table~\ref{tab:rq1-failure}). Full numeric grids appear in Tables~\ref{tab:cail_main}--\ref{tab:lawyer_main}.

\begin{figure}[t]
    \centering
\includegraphics[width=0.95\linewidth]{fig3_cail_main.pdf}
\caption{CAIL2024 multi-turn results at tier $M\approx 3000$. Soft~O2 improves AH and EC over FlatIP; shuffled $sid$ removes the gain; hard hydration can further raise AH/EC.}
\label{fig:cail_main}
\end{figure}

Figure~\ref{fig:legalep_main} reports LegalEp transfer on the same AH grids.
Soft~O2 also raises exact replay (DISC $0.916$ vs FlatIP $0.714$; Lawyer $0.976$ vs $0.678$), which we treat as diagnostic relative to paraphrase and advice-recall.
Paraphrase gains are large: $0.883$ vs $0.696$ on DISC and $0.956$ vs $0.697$ on Lawyer.
Advice-recall rises to $0.610$ from FlatIP $0.472$ on DISC and to $0.654$ from $0.416$ on Lawyer.
Hard hydration can still edge Soft~O2 on these channels; shuffled $sid$ collapses toward FlatIP.
...
Cross-encoder reranking is a strong neural control at the same turn-level evidence unit: FlatIP+CE reranks the top-$30$ FlatIP candidates with \modelid{bge-reranker-v2-m3}.
Table~\ref{tab:ce} reports a secondary CE campaign (separate from the unified Soft~O2 rebuild): within that campaign FlatIP+CE is strongest on LegalEp exact channels, while Soft~O2 leads on CAIL multi-turn channels (Uk $0.698$ vs CE $0.375$; U-last $0.762$ vs $0.342$).
...
Soft~O2 recovers such siblings by propagating attenuated scores through session links, including cases where the stored advice turn has weak direct lexical overlap with the query (paraphrase and advice-recall; Tables~\ref{tab:soft-o2-controls} and~\ref{tab:ndcg-graded}).

\begin{table}[t]
\centering
\caption{Secondary cross-encoder control (AH@10; CE campaign store). FlatIP+CE reranks the top-$30$ FlatIP candidates with a multilingual cross-encoder. Soft~O2 column is from the same CE campaign for within-table comparison; primary Soft~O2 vs FlatIP AH uses the unified rebuild (Tables~\ref{tab:soft-o2-controls}--\ref{tab:lawyer_main}). Boldface: best AH within each row.}
\label{tab:ce}
\compacttab\footnotesize
\begin{tabular}{@{}llccc@{}}
\toprule
...
LegalEp-DISC / exact & 0.572 & \textbf{0.722} & 0.572 \\
LegalEp-Lawyer / exact & 0.636 & \textbf{0.708} & 0.636 \\
\bottomrule
\end{tabular}
\end{table}
...
Hybrid uses a direct-dense gate: cluster binding fires only on turn ids recorded \emph{before} Soft~O2 session expansion.

On LegalEp Mix stores, gated Hybrid is the strongest overall system (DISC AH $0.542$, Lawyer $0.536$; McNemar mid-$p{=}0.0005^{**}$ / $0.0022^{**}$ vs Soft~O2) and also leads $\mathrm{AH}_{\mathrm{cross}}$.
Ungated Soft~O2-C raises DISC $\mathrm{AH}_{\mathrm{cross}}$ to $0.506$ from Soft~O2 $0.460$ and Lawyer cross to $0.480$ from $0.440$, but remains weaker than Hybrid once session binding is composed under the gate.
On LegalMem-MT Mix the picture reverses: Soft~O2 retains the best overall AH ($0.602$), while Soft~O2-C ($0.440$) and Hybrid ($0.562$) trail; FlatIP even leads the cross stratum ($0.520$).
Cluster binding is therefore conditional on corpus and cluster purity---helpful as a gated add-on on LegalEp advice-recall Mix, but not uniformly beneficial on CAIL multi-turn Mix under the same protocol.
Same-session Soft~O2 vs Soft~O2-C contrasts remain Soft~O2-led (Appendix~\ref{app:tables}, Table~\ref{tab:cluster_same}).

\begin{table}[htbp]
\centering
\caption{Mix evaluation of Soft~O2-C and gated Hybrid with the direct-dense cluster trigger (same store, unrestricted dense; AH@10). Gold is $70$\% cross-session Sa/Sb + $30$\% intact same-session. Boldface: best overall AH and best cross-session AH within each corpus. Significance vs Soft~O2: ${}^{*}\,p{<}0.05$, ${}^{**}\,p{<}0.01$.}
\label{tab:csce}
\compacttab\footnotesize
\begin{tabular}{@{}llcccc@{}}
\toprule
...
\multirow{4}{*}{LegalEp-DISC Mix}
 & FlatIP & 0.494 & 0.491 & 0.500 & --- \\
 & Soft O2 & 0.492 & 0.460 & 0.567 & --- \\
 & Soft O2-C & 0.498 & 0.506 & 0.480 & 0.7275 \\
 & Hybrid & \textbf{0.542} & \textbf{0.526} & \textbf{0.580} & 0.0005${}^{**}$ \\
\midrule
\multirow{4}{*}{LegalEp-Lawyer Mix}
 & FlatIP & 0.460 & 0.451 & 0.480 & --- \\
 & Soft O2 & 0.492 & 0.440 & \textbf{0.613} & --- \\
 & Soft O2-C & 0.464 & 0.480 & 0.427 & 0.1284 \\
 & Hybrid & \textbf{0.536} & \textbf{0.520} & 0.573 & 0.0022${}^{**}$ \\
\midrule
\multirow{4}{*}{LegalMem-MT Mix}
 & FlatIP & 0.530 & \textbf{0.520} & 0.553 & --- \\
 & Soft O2 & \textbf{0.602} & 0.489 & \textbf{0.867} & --- \\
 & Soft O2-C & 0.440 & 0.460 & 0.393 & 0.0000${}^{**}$ \\
 & Hybrid & 0.562 & 0.446 & 0.833 & 0.0594 \\
\bottomrule
\end{tabular}
\end{table}
{\footnotesize Hybrid uses the direct-dense gate. Soft~O2-C mid-$p$ vs Soft~O2 may favor either side; on LegalMem-MT the ${}^{**}$ marks Soft~O2-C significantly \emph{below} Soft~O2. Cluster co-membership of Sa/Sb pairs follows Section~\ref{sec:csce-protocol}.}

\subsection{Scale behaviour}\label{sec:exp-scale}
Figure~\ref{fig:scale} and Table~\ref{tab:scale} report quality and warm-query latency
on the LegalEp DISC exact channel ($Q{=}100$; three repeats; archive sizes
$100$, $400$, $1600$, and $6400$).
Soft~O2 retains an AH advantage at each tier.
IVFPQ collapses at $M{=}1600$ (AH $0.330$), which motivates O1, while absolute latency grows with $M$.
We treat O1 FlatIP as an engineering prerequisite for Soft~O2 at these scales
and do not claim deployment readiness beyond the measured curve.
...
Episode incompleteness is a substantial FlatIP failure under same-domain interference; Soft~O2 converts many incomplete retrievals into complete ones without shrinking session-miss rates (Table~\ref{tab:rq1-failure}).
...
The gap-ratio band $\rho{<}\beta{<}1$ and the AH sensitivity plateau $\{0.9,0.95,0.98\}$ jointly motivate freezing $\beta{=}0.98$ below hard hydration.
Primary Soft~O2 vs FlatIP gains on CAIL and LegalEp remain Holm-significant (Table~\ref{tab:sig}); paraphrase and advice-recall are the main transfer channels, with graded nDCG agreeing (Tables~\ref{tab:soft-o2-controls}--\ref{tab:ndcg-graded}).
Under Mix, gated Hybrid improves overall AH over Soft~O2 on LegalEp-DISC and LegalEp-Lawyer, while Soft~O2 remains strongest on LegalMem-MT and on intact same-session gold (Tables~\ref{tab:csce}--\ref{tab:cluster_same}).
Turn-level BM25, RRF, and CE place Soft~O2 among standard IR alternatives \cite{robertson2009bm25, cormack2009rrf, nogueira2019multistage}; joint-episode indexes remain a coarser design outside this protocol.
...
Per corpus we pool $N{=}270$ judged instances as P1~$n{=}120$ plus P2~$n{=}150$.
...
Hallucinated-legal-basis rates ($0.117$~/~$0.133$) remain non-zero: grounding in retrieved episodes does not guarantee statutory correctness.
...
That design lies outside the Soft~O2 turn-binding protocol.
A matched comparison under online updates, latency, and fine-grained advice isolation is left to future work.

\paragraph{Embedding model and other scope limits.}
All main grids use \modelid{nomic-embed-text-v2-moe}.
A Chinese-specialized encoder such as \modelid{bge-large-zh-v1.5} could alter absolute dense ranks and the measured $\rho$ distribution, and might shrink or widen Soft~O2's relative gain wherever the bi-encoder already places answer turns inside top-$k$.
...
Because many channels are tested, Holm-adjusted $p$-values accompany the Soft~O2 vs FlatIP family (Table~\ref{tab:sig}); Mix Hybrid significance is reported only vs Soft~O2 within Table~\ref{tab:csce} and is corpus-conditional.

\paragraph{Latency and deployment scope.}
Scale curves cover archive sizes $100$, $400$, $1600$, and $6400$ under a fixed hardware stack (Table~\ref{tab:scale}).
...
BIMS-LEGAL is therefore best positioned for high-precision offline or near-real-time consultation-memory recovery---for example, pre-meeting case review, audit replay, or batch RAG over a firm's advice archive---rather than as a sub-millisecond search engine.
...
Soft~O2 completes missing siblings by attenuated session binding ($\beta{=}0.98$), selected inside the gap-ratio band via an Answer Hit sensitivity sweep (Table~\ref{tab:beta}); hard hydration remains a stronger unattenuated upper bound.
Soft~O2-C extends the same operator to thematic clusters under Mix reconstructions as a conditional mechanism test, with O1 FlatIP and O3 bulk-load as the shared-store implementation operators.
On CAIL multi-turn and LegalEp paraphrase~/advice-recall channels, Soft~O2 improves Answer Hit and graded nDCG over FlatIP and cuts incompleteness; shuffled-$sid$ collapses the Soft~O2 gain.
Under Mix, gated Hybrid recovers additional LegalEp gold when cluster co-membership is present, while Soft~O2 remains preferable on LegalMem-MT Mix and on intact same-session gold.
A Soft~O2 QA audit links complete-episode retrieval to judged answerability with low wrong-session contamination; it is not a paired FlatIP vs Soft~O2 generation comparison.
...
Code, processed evaluation corpora, and primary Soft~O2 metric artifacts (including paired per-query Answer Hit used for Holm tests) are available at
...
The BIMS-LEGAL implementation (O1 FlatIP, Soft~O2, O3 bulk-load, Soft~O2-C), corpus builders, Mix constructors, and the scripts that regenerate Soft~O2 tables, figures, and Holm tests from one metric family ship in the same repository.
...
\begin{table}[H]
\centering
\caption{Primary Soft~O2 vs FlatIP family: McNemar mid-$p$ on paired Answer Hit from the unified FlatIP rebuild, with Holm step-down adjustment ($m{=}9$). Each $\Delta$AH matches Soft~O2$-$FlatIP in the main Soft~O2 tables; paired outcomes are released with the paper artifacts.}
\label{tab:sig}
\compacttab\footnotesize
\begin{tabular}{@{}llccc@{}}
\toprule
...
\midrule
CAIL / Uk & O2 vs FlatIP & +0.500 & 3.69e-69 & 2.95e-68 \\
CAIL / U1 & O2 vs FlatIP & +0.145 & 1.15e-16 & 3.44e-16 \\
CAIL / U-last & O2 vs FlatIP & +0.563 & 3.19e-85 & 2.87e-84 \\
LegalEp-DISC / exact & O2 vs FlatIP & +0.202 & 2.32e-19 & 9.28e-19 \\
LegalEp-Lawyer / exact & O2 vs FlatIP & +0.298 & 1.16e-39 & 8.12e-39 \\
LegalEp-DISC / u-para & O2 vs FlatIP & +0.187 & 1.28e-16 & 3.44e-16 \\
LegalEp-Lawyer / u-para & O2 vs FlatIP & +0.259 & 2.07e-32 & 1.04e-31 \\
LegalEp-DISC / advice & O2 vs FlatIP & +0.138 & 1.52e-15 & 1.52e-15 \\
LegalEp-Lawyer / advice & O2 vs FlatIP & +0.238 & 7.84e-33 & 4.70e-32 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{Soft~O2 AH@10 sensitivity sweep over $\beta\in\{0.5,0.7,0.9,0.95,0.98,1.0\}$ on the CAIL Uk-followup and LegalEp exact/paraphrase evaluation channels (LegalMem-MT $\beta$-campaign stores; not held-out). Used with the gap-ratio analysis to freeze default $\beta{=}0.98$. Peak AH lies in $\{0.9,0.95,0.98\}$; $\beta{=}0.5$ weakens binding. Not a primary Soft~O2 result table; absolute AH values are not comparable to the unified FlatIP-rebuild Tables~\ref{tab:soft-o2-controls}--\ref{tab:lawyer_main}.}
\label{tab:beta}
\compacttab\footnotesize
...
\midrule
\multirow{4}{*}{U1}
 & FlatIP & 0.803 & 0.544 & 0.538 \\
 & Soft O2 & 0.948 & 0.918 & 0.767 \\
 & Hard hydr. & \textbf{0.995} & \textbf{0.994} & 0.859 \\
 & Shuffled O2 & 0.660 & 0.427 & 0.393 \\
\midrule
\multirow{4}{*}{Uk-followup}
 & FlatIP & 0.432 & 0.427 & 0.428 \\
 & Soft O2 & 0.932 & 0.925 & 0.796 \\
 & Hard hydr. & \textbf{0.997} & \textbf{0.994} & 0.853 \\
 & Shuffled O2 & 0.313 & 0.348 & 0.349 \\
\midrule
\multirow{4}{*}{U-last}
 & FlatIP & 0.393 & 0.415 & 0.400 \\
 & Soft O2 & 0.957 & 0.933 & 0.796 \\
 & Hard hydr. & \textbf{0.998} & \textbf{0.995} & 0.861 \\
 & Shuffled O2 & 0.255 & 0.340 & 0.326 \\
\bottomrule
\end{tabular}
\end{table}
{\footnotesize Soft~O2 improves AH/EC over FlatIP; hard hydration uses unrestricted score copy and can lead both.}
...
\midrule
\multirow{4}{*}{exact}
 & FlatIP & 0.714 & 0.857 & 0.700 \\
 & Soft O2 & 0.916 & 0.958 & 0.792 \\
 & Hard hydr. & \textbf{1.000} & \textbf{1.000} & 0.873 \\
 & Shuffled O2 & 0.642 & 0.821 & 0.626 \\
\midrule
\multirow{4}{*}{advice-recall}
 & FlatIP & 0.472 & 0.560 & 0.454 \\
 & Soft O2 & 0.610 & 0.628 & 0.518 \\
 & Hard hydr. & \textbf{0.648} & \textbf{0.648} & 0.563 \\
 & Shuffled O2 & 0.434 & 0.540 & 0.411 \\
\midrule
\multirow{4}{*}{u-para}
 & FlatIP & 0.696 & 0.836 & 0.659 \\
 & Soft O2 & 0.883 & 0.932 & 0.742 \\
 & Hard hydr. & \textbf{0.972} & \textbf{0.972} & 0.816 \\
 & Shuffled O2 & 0.620 & 0.797 & 0.590 \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
\multirow{4}{*}{exact}
 & FlatIP & 0.678 & 0.839 & 0.710 \\
 & Soft O2 & 0.976 & 0.988 & 0.851 \\
 & Hard hydr. & \textbf{1.000} & \textbf{1.000} & 0.880 \\
 & Shuffled O2 & 0.654 & 0.827 & 0.656 \\
\midrule
\multirow{4}{*}{advice-recall}
 & FlatIP & 0.416 & 0.546 & 0.457 \\
 & Soft O2 & 0.654 & 0.664 & 0.569 \\
 & Hard hydr. & \textbf{0.674} & \textbf{0.674} & 0.591 \\
 & Shuffled O2 & 0.396 & 0.535 & 0.424 \\
\midrule
\multirow{4}{*}{u-para}
 & FlatIP & 0.697 & 0.830 & 0.708 \\
 & Soft O2 & 0.956 & 0.965 & 0.832 \\
 & Hard hydr. & \textbf{0.978} & \textbf{0.978} & 0.847 \\
 & Shuffled O2 & 0.675 & 0.815 & 0.656 \\
\bottomrule
\end{tabular}
\end{table}
