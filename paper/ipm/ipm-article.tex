...
The primary hypothesis family is Soft~O2 vs FlatIP on the nine CAIL and LegalEp transfer channels; for that family we report McNemar mid-$p$ on paired per-query Answer Hit and Holm-adjusted $p$-values (Table~\ref{tab:sig}), computed from the same unified FlatIP rebuild as the main Soft~O2 grids.
...
Unless noted, Soft~O2 grids in Sections~\ref{sec:s1-strong-baselines}--\ref{sec:exp-main} and Appendix Tables~\ref{tab:cail_main}--\ref{tab:lawyer_main} use one FlatIP rebuild so that AH, EC, nDCG, failure rates, and the Table~\ref{tab:sig} paired outcomes on each row come from the same per-query rankings (released metric JSON artifacts, including per-query Answer Hit vectors).
...
Under the unified rebuild Soft~O2 also lifts exact replay (DISC $0.924$ vs FlatIP $0.742$; Lawyer $0.976$ vs $0.732$), but we still treat exact as diagnostic relative to paraphrase and advice-recall.
Paraphrase yields large Soft~O2 gains over FlatIP: $0.895$ vs $0.716$ on DISC and $0.946$ vs $0.739$ on Lawyer.
