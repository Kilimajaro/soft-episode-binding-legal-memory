...
\begin{table}[t]
\centering
\caption{RQ1 failure taxonomy ($k{=}10$) from the FlatIP rebuild with fixed-gold metrics. Incomplete = target session partially retrieved but answer missing. Soft~O2 sharply reduces incompleteness while session-miss rates stay largely unchanged, motivating session soft binding before Soft~O2-C.}
...
LegalEp-DISC / u-para & Soft O2 & \textbf{89.1\%} & \textbf{8.7\%} & 1.8\% \\
LegalEp-DISC / advice & FlatIP & 48.0\% & 17.0\% & 35.0\% \\
LegalEp-DISC / advice & Soft O2 & \textbf{60.8\%} & \textbf{3.8\%} & 35.4\% \\
LegalEp-Lawyer / u-para & FlatIP & 71.5\% & 25.3\% & 0.8\% \\
LegalEp-Lawyer / u-para & Soft O2 & \textbf{94.0\%} & \textbf{3.8\%} & 1.6\% \\
CAIL / Uk-followup & FlatIP & 45.8\% & 54.0\% & 0.2\% \\
CAIL / Uk-followup & Soft O2 & \textbf{92.8\%} & \textbf{7.0\%} & 0.2\% \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Turn-level lexical controls}\label{sec:s1-strong-baselines}
Primary Soft~O2 claims stay within a turn-level index: FlatIP, Soft~O2, hard hydration, and shuffled $sid$ share the same episodic store and evidence unit.
As a same-granularity lexical control, BM25-turn indexes each turn independently (Table~\ref{tab:bm25}).
On CAIL non-exact channels Soft~O2 exceeds BM25-turn (Uk $0.698$ vs $0.482$; U-last $0.762$ vs $0.413$), while BM25-turn remains competitive on exact replay where surface overlap is high.
We deliberately omit BM25-joint and dense joint-episode indexing from Soft~O2 head-to-head tables: both concatenate question and answer into one document before retrieval, so a hit already contains the answer and the comparison would confound Soft~O2's binding mechanism with a change of input granularity.
Those coarser-grained designs are left as future work rather than Soft~O2 baselines.

\begin{table}[t]
\centering
\caption{Corrected graded nDCG@10 (fixed-gold IDCG) under a fresh FlatIP rebuild, distinct from the published V4 AH/EC grids in Tables~\ref{tab:cail_main}--\ref{tab:lawyer_main}. Boldface: Soft~O2 better than FlatIP within each corpus/channel pair on AH and nDCG. Soft~O2 reduces incompleteness while raising both metrics.}
...
CAIL & U1 & Soft O2 & \textbf{0.960} & \textbf{0.776} & 4.0\% \\
...
LegalEp-DISC & exact & Soft O2 & \textbf{0.924} & \textbf{0.813} & 7.6\% \\
...
LegalEp-Lawyer & exact & Soft O2 & \textbf{0.976} & \textbf{0.869} & 2.4\% \\
...
Figure~\ref{fig:cail_main} summarizes published V4 AH@10 and EC@10 on authentic CAIL multi-turn channels at $M\approx 3000$ under O1 FlatIP---these absolute AH levels are the Soft~O2 transfer claims used throughout the paper.
...
Under corrected fixed-gold IDCG on a fresh FlatIP rebuild (Table~\ref{tab:ndcg-corrected}; absolute AH levels differ from V4), Soft~O2 also raises nDCG over FlatIP on every CAIL channel (U1 $0.776$ vs $0.554$; Uk $0.793$ vs $0.445$; U-last $0.791$ vs $0.420$) while cutting incompleteness sharply (Table~\ref{tab:rq1-failure}).
...
Together, Tables~\ref{tab:cluster_same}--\ref{tab:csce} support a staged narrative: Soft~O2 is the default same-session operator; Soft~O2-C~/Hybrid are exploratory complements that can recover additional cross-session gold under the Mix solvability-bound protocol while Soft~O2 remains strong on intact same-session gold.

\begin{table}[htbp]
\centering
\caption{Fair Mix protocol (same store, unrestricted dense; AH@10). Gold is a $70\%$/$30\%$ mix of cross-session Sa/Sb splits and intact same-session episodes. Soft~O2-C denotes ungated cluster binding; Hybrid is Soft~O2+gated Soft~O2-C. Boldface: best overall AH within each corpus (and best cross-session AH when Hybrid uniquely leads that stratum). Significance vs Soft~O2: ${}^{*}\,p{<}0.05$, ${}^{**}\,p{<}0.01$.}
\label{tab:csce}
\compacttab\footnotesize
...
RQ3 shows that the BIRCH slow pathway benefits Soft~O2-C only when gold is not already co-session; Mix Hybrid Soft~O2+Soft~O2-C recovers that regime without forcing Soft~O2 to zero on intact same-session gold.
RQ4: Soft~O2 (session) is the default operator; Soft~O2-C~/Hybrid (cluster) are complementary exploratory bounds within one BIMS-LEGAL dual-store.
