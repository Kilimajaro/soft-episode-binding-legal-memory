...
We formalize Soft~O2 as a three-candidate ranking-competition operator and derive a corpus-adaptive $\beta^{\star}$ from pairwise hinge and soft-rank losses; fitted gap ratios on LegalEp stores place $\beta^{\star}$ in $\{0.95,0.98\}$, matching the empirical Answer-Hit peak band.
...
\begin{enumerate}
    ...
    \item A ranking-competition derivation of the Soft~O2 attenuation $\beta$ from pairwise hinge and soft-rank objectives on a three-candidate (hit~/sibling~/distractor) model, with gap-ratio fits that recover the empirical $\beta\in\{0.9,0.95,0.98\}$ operating band.
...
Section~\ref{sec:method} presents BIMS-LEGAL with formal dual-store operators, Soft~O2~/Soft~O2-C~/Hybrid algorithms, and the $\beta^{\star}$ derivation.
...
\end{equation}
where $S$ is cosine similarity, $R(t)=\exp(-\lambda\Delta t)$ is an exponential recency prior, and $(\alpha,\lambda)=(0.7,\lambda_0)$ by default with $\alpha$ the semantic weight.
Eq.~\eqref{eq:base-score} is the ranking score used throughout the legal grids; Soft~O2~/Soft~O2-C are subsequent binding operators and do not replace the base score.
...
\subsection{Ranking-competition derivation of $\beta$}\label{sec:beta-theory}
We lift $\beta$ from a heuristic near-unity constant to the optimum of an explicit ranking-competition loss on a minimal three-candidate model.

\begin{definition}[Three-candidate episode model]\label{def:three-cand}
For a query that densily retrieves a gold question turn $h$ (the \emph{hit}) with score $s_h>0$, let $a$ be the gold answer sibling and $d$ the strongest same-domain distractor outside $sid_h$, with dense scores $s_a$ and $s_d$.
Soft~O2 rewrites only the sibling:
\begin{equation}\label{eq:soft-rewrite}
s'_h=s_h,\qquad s'_a=\max(s_a,\beta s_h),\qquad s'_d=s_d.
\end{equation}
Define the relative interference (gap ratio)
\begin{equation}\label{eq:rho}
\rho=\frac{s_d}{s_h}\in\mathbb{R}_{+}.
\end{equation}
When $s_a$ lies below the competitive fringe (the incompleteness regime), entry of $a$ into top-$k$ against $d$ requires $\beta s_h>s_d$, i.e.\ $\beta>\rho$, while strict rank competition with the hit requires $\beta<1$.
\end{definition}

\paragraph{Pairwise hinge objective.}
With availability margin $m_{\mathrm{a}}\ge 0$ and rank margin $m_{\mathrm{r}}\ge 0$, the per-query hinge loss is
\begin{equation}\label{eq:hinge}
\begin{aligned}
\mathcal{L}_{\mathrm{hinge}}(\beta;\rho)
&=w_{\mathrm{a}}\bigl[m_{\mathrm{a}}-(\beta-\rho)\bigr]_{+}
+w_{\mathrm{r}}\bigl[m_{\mathrm{r}}-(1-\beta)\bigr]_{+},
\end{aligned}
\end{equation}
where $[\cdot]_{+}=\max(\cdot,0)$ and $(w_{\mathrm{a}},w_{\mathrm{r}})$ trade Answer-Hit availability against hit-first rank preservation \cite{burges2005learning,joachims2002optimizing}.
For $m_{\mathrm{a}}=m_{\mathrm{r}}=0$ the population risk $\mathbb{E}_{\rho}[\mathcal{L}_{\mathrm{hinge}}]$ is minimized by covering a $\pi$-fraction of interference cases,
\begin{equation}\label{eq:beta-hinge}
\beta^{\star}_{\mathrm{hinge}}
=\mathrm{clip}\!\left(\mathrm{Quantile}_{\pi}(\rho),\,0,\,1\right),
\qquad
\pi=\frac{w_{\mathrm{a}}}{w_{\mathrm{a}}+w_{\mathrm{r}}}.
\end{equation}
Thus $\beta^{\star}$ is an explicit functional of the gap-ratio law, top-$k$ fringe (via how $d$ is chosen), and the availability--ranking utility weights---not an arbitrary constant.

\paragraph{Soft-rank / logistic competition.}
Following soft pairwise ranking \cite{taylor2008softrank,cao2007learning}, define
...
\end{equation}
with logistic $\sigma(x)=(1+e^{-x})^{-1}$, temperature $\tau>0$, and rank weight $\gamma>0$.
The first term soft-encourages $a\succ d$ under Soft~O2; the second soft-encourages $h\succ a$ (rank competition).
The corpus-level estimator is
\begin{equation}\label{eq:beta-sr}
\beta^{\star}_{\mathrm{sr}}
=\arg\min_{\beta\in(0,1]}\ \mathbb{E}_{\rho}\bigl[\mathcal{L}_{\mathrm{sr}}(\beta;\rho)\bigr],
\end{equation}
computed on a fine grid (released analysis script under \path{paper/scripts/}).

\begin{proposition}[Feasible Soft~O2 band]\label{prop:feasible}
Under Definition~\ref{def:three-cand}, if $\rho<1$ then every $\beta\in(\rho,1)$ simultaneously (i)~places the soft-injected sibling strictly above the measured distractor and (ii)~keeps the dense hit strictly above the sibling.
Hard hydration ($\beta{=}1$) collapses (ii) whenever $s_a$ is replaced by $s_h$, removing within-episode rank competition; aggressive attenuation ($\beta\le\rho$) fails (i) and cannot repair incompleteness against $d$.
\end{proposition}

\begin{remark}[Semi-empirical calibration]
Fitting $\rho$ on LegalEp FlatIP stores (query proxy: stored gold user-turn embedding; $d$: strongest non-episode competitor in a wide pool) yields median $\rho\approx 0.75$~/~$0.66$ on DISC~/Lawyer needles, with $\mathrm{Quantile}_{0.95}(\rho)\approx 0.85$~/~$0.82$.
Soft-rank optima at $(\tau,\gamma)=(20,0.05)$ give $\beta^{\star}_{\mathrm{sr}}\approx 0.987$~/~$0.949$ (mean $0.968$), squarely inside the empirical AH peak band $\{0.9,0.95,0.98\}$ of Table~\ref{tab:beta}; coverage $\Pr(\rho\le 0.90)$ exceeds $0.98$ on both corpora (Table~\ref{tab:beta-theory}).
Default $\beta{=}0.98$ is therefore the soft-rank operating point under light rank regularization, not an unexplained heuristic.
\end{remark}

\begin{table}[t]
\centering
\caption{Gap-ratio fit versus Soft~O2 $\beta$ on LegalEp $M{=}3000$ stores. $\beta^{\star}_{\mathrm{sr}}$: soft-rank optimum ($\tau{=}20$, $\gamma{=}0.05$). Cover: fraction of gold episodes with $\rho\le\beta$.}
\label{tab:beta-theory}
\compacttab\footnotesize
\begin{tabular}{@{}lcccccc@{}}
\toprule
\textbf{Corpus} & $\rho_{50}$ & $\rho_{95}$ & $\beta^{\star}_{\mathrm{sr}}$ & Cover@$0.90$ & Cover@$0.98$ & Default $\beta$ \\
\midrule
...
A $\beta$ sweep peaks near $\{0.9,0.95,0.98\}$ and weakens at $\beta{=}0.5$ (Table~\ref{tab:beta}), in agreement with the soft-rank $\beta^{\star}$ and gap-ratio coverage in Section~\ref{sec:beta-theory} (Table~\ref{tab:beta-theory}).
...
The $\beta^{\star}$ derivation is semi-empirical: it assumes a dominant distractor and a well-defined hit score, and the fitted $\rho$ uses a stored user-turn embedding as an exact-channel query proxy; paraphrase~/advice-recall gap laws may shift $\beta^{\star}$ slightly within the same high band.
