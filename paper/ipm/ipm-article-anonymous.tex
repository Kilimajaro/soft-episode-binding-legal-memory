...
\begin{abstract}
\sloppy
Recovering prior client-specific advice from a legal consultation archive is difficult: dense retrieval may rank a relevant question highly while omitting the lawyer's answer, an \emph{episode incompleteness} failure under same-domain interference.
...
We characterize that trade-off with a three-candidate score-competition model and choose $\beta$ by validation sweep rather than by a closed-form optimum.
...
If recovery still fails, the failure is attributable to binding rather than accidental cluster separation.
...
Unless noted, the embedding model is \texttt{nomic-embed-text-v2-moe} \cite{nussbaum2025nomic}, $k{=}10$, and the main $M{\approx}3000$ grids use seed $42$.
A five-seed check on a smaller shared store ($M{=}400$) is reported later; robustness across alternative encoders is left open (Section~\ref{sec:limitations}).
Primary contrasts appear in figures; full numeric grids are in the appendix.
...
The Mix protocol assigns Sa/Sb pairs a shared cluster id so Soft~O2-C can be tested when thematic co-membership is present (Section~\ref{sec:csce-protocol}); unsupervised BIRCH without that construction would not receive split-pair metadata at build time.
Soft~O2 depends on correct session identifiers; Soft~O2-C depends on cluster quality.
Joint-episode document indexes change the evidence unit from turns to concatenated episodes; comparing them under a matched online-update and latency protocol is left to future work.
...
All main grids use one multilingual embedding model and a fixed seed at $M{\approx}3000$; five-seed variance is reported only for the smaller $M{=}400$ store.
