...
The Soft~O2 point estimates in Sections~\ref{sec:s1-strong-baselines}--\ref{sec:exp-main} and Appendix Tables~\ref{tab:cail_main}--\ref{tab:lawyer_main} come from one FlatIP rebuild (released \texttt{corrected\_metrics\_*.json}).
Paired McNemar/Holm outcomes are taken from the \texttt{per\_query\_ah} fields of those artifacts once exported, so Table~\ref{tab:sig} $\Delta$AH tracks Soft~O2$-$FlatIP on the Soft~O2 grids.
...
\begin{table}[H]
\centering
\caption{Primary Soft~O2 vs FlatIP family: McNemar mid-$p$ on paired Answer Hit with Holm step-down adjustment ($m{=}9$). $\Delta$AH and $p$-values are computed from the released \texttt{per\_query\_ah} vectors in the Soft~O2 metric JSON artifacts.}
\label{tab:sig}
\compacttab\footnotesize
\begin{tabular}{@{}llccc@{}}
\toprule
