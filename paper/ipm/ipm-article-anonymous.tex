...
Cluster binding is therefore conditional on corpus and cluster purity---helpful as a gated add-on on LegalEp advice-recall Mix, but not a free lunch on CAIL multi-turn Mix under the same protocol.
Same-session Soft~O2 vs Soft~O2-C contrasts remain Soft~O2-led (Appendix~\ref{app:tables}, Table~\ref{tab:cluster_same}).

\begin{table}[htbp]
\centering
\caption{Mix evaluation of Soft~O2-C and gated Hybrid after the direct-dense gate fix (same store, unrestricted dense; AH@10). Gold is $70$\% cross-session Sa/Sb + $30$\% intact same-session. Boldface: best overall AH and best cross-session AH within each corpus. Significance vs Soft~O2: ${}^{*}\,p{<}0.05$, ${}^{**}\,p{<}0.01$.}
\label{tab:csce}
\compacttab\footnotesize
\begin{tabular}{@{}llcccc@{}}
\toprule
