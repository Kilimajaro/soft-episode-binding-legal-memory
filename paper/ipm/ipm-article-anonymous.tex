...
A fast episodic pathway indexes timestamped turn embeddings with speaker role and session identifier $sid$; a slower semantic pathway consolidates turns with Balanced Iterative Reducing and Clustering using Hierarchies (BIRCH) \cite{zhang1996birch}.
...
On the semantic pathway, Soft~O2-C applies the same rule within BIRCH clusters when gold evidence is not co-session with the query, and a gated Hybrid triggers cluster binding only on direct dense hits.
...
We study how often episode incompleteness occurs relative to full session miss, how large Soft~O2 gains are against FlatIP and hard hydration on paraphrase, follow-up, and advice-recall channels, and when Soft~O2-C~/Hybrid helps under Mix reconstructions with off-session gold.
...
Third, we evaluate Soft~O2 on CAIL multi-turn and rebuilt LegalEp corpora against FlatIP, hard hydration, shuffled-$sid$, BM25-turn, RRF, and cross-encoder controls, and examine Soft~O2-C~/Hybrid under Mix cross-session reconstructions with graded nDCG, a failure taxonomy, and paired significance tests.
...
\begin{table}[t]
\centering
\caption{Failure taxonomy ($k{=}10$). Boldface: best complete / lowest incomplete within each corpus--channel block among FlatIP, Soft~O2, and shuffled~$sid$ (hard hydration shown as a score-copy envelope). Soft~O2 cuts incompleteness without raising session miss; shuffled~$sid$ undoes the gain.}
...
Shuffled $sid$ removes the gain (U1 $0.452$; Uk $0.230$; U-last $0.200$), tying the improvement to correct episode membership.
...
Code and processed evaluation artifacts will be released upon publication.
Upstream corpora remain under their original licences: CAIL2024 consultation tracks; Hugging Face
\codeid{ShengbinYue/DISC-Law-SFT} and
\codeid{Skepsun/lawyer_llama_data}.
Processed LegalEp~/LegalMem artifacts are derived research releases.

\subsection*{Code availability}
Implementation artifacts and per-table JSON files will be released with the camera-ready version.
