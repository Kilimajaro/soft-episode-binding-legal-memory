...
We report Answer Hit, Episode Completeness, graded nDCG with fixed gold-session IDCG, a four-way failure taxonomy, and Holm-adjusted Soft~O2 vs FlatIP tests whose $\Delta$AH matches the Soft~O2 grids by construction.
\end{abstract}
...
Second, we propose Soft~O2 as a turn-level soft-binding operator: a gap-ratio score-competition analysis identifies a feasible high-$\beta$ band, and an Answer Hit grid search over $\{0.5,0.7,0.9,0.95,0.98,1.0\}$ selects default $\beta{=}0.98$ (Table~\ref{tab:beta}), with O1/O3 as implementation operators for shared-store evaluation.
...
We therefore characterize the availability--ranking trade-off with a three-candidate score-competition model and then choose $\beta$ by an empirical Answer Hit grid search inside that band, rather than by a closed-form optimum alone.
A zero-margin hinge objective on $\beta\in(0,1]$ is uninformative as a selector: its rank term vanishes for all $\beta<1$.
...
\paragraph{Soft-rank visualization and AH grid search.}
...
The curve visualizes the availability--ranking trade-off under the measured $\rho$ pool and is not itself the parameter selector.
Default $\beta{=}0.98$ is chosen by an Answer Hit grid search over $\{0.5,0.7,0.9,0.95,0.98,1.0\}$ on CAIL follow-up and LegalEp exact/paraphrase channels (Table~\ref{tab:beta}): the high-AH plateau $\{0.90,0.95,0.98\}$ matches the theoretically preferred band $\rho{<}\beta{<}1$, while hard hydration ($\beta{=}1$) is not better and aggressive attenuation ($\beta{=}0.5$) collapses binding.
That grid-search table is a selection/sensitivity diagnostic and is not the primary Soft~O2 result grids in Tables~\ref{tab:soft-o2-controls}--\ref{tab:lawyer_main}, which freeze $\beta{=}0.98$ under the unified FlatIP rebuild; absolute AH values are therefore not cross-table comparable.
Advice-recall uses the same selected default without a separate gap-ratio fit.
...
Those paired outcomes are taken from the same unified FlatIP rebuild that supplies the Soft~O2 grids, so a reader can regenerate both the point estimates and the significance tests from one artifact family.
...
Unless noted, Soft~O2 grids in Sections~\ref{sec:s1-strong-baselines}--\ref{sec:exp-main} and Appendix Tables~\ref{tab:cail_main}--\ref{tab:lawyer_main} use one FlatIP rebuild so that AH, EC, nDCG, failure rates, and the Table~\ref{tab:sig} paired outcomes on each row come from the same per-query rankings.
Consequently, each $\Delta$AH in Table~\ref{tab:sig} equals the corresponding Soft~O2$-$FlatIP difference in the main Soft~O2 tables (up to ordinary rounding).
...
Hard hydration---full sibling score copy---can exceed Soft~O2 AH on the same rebuild (Table~\ref{tab:soft-o2-controls}); Soft~O2 is best read as an attenuated binder that recovers missing answers without unrestricted score copy (Tables~\ref{tab:cail_main}--\ref{tab:lawyer_main}).
...
Soft~O2 lifts Answer Hit over FlatIP on every channel under the unified rebuild (U1 $0.948$ vs $0.803$; Uk $0.932$ vs $0.432$; U-last $0.957$ vs $0.393$).
Shuffled $sid$ removes the gain, tying the improvement to correct episode membership.
Hard expansion can raise AH further by unrestricted score copy (e.g., Uk $0.997$ vs Soft~O2 $0.932$).
Soft~O2 is the attenuated binder: siblings enter under $\beta{<}1$ scores, whereas hard hydration copies sibling scores without attenuation.
...
\caption{CAIL2024 multi-turn results at tier $M\approx 3000$. Soft~O2 improves AH and EC over FlatIP; shuffled $sid$ removes the gain; hard expansion can further raise AH/EC by unrestricted score copy.}
\label{fig:cail_main}
\end{figure}

Figure~\ref{fig:legalep_main} reports LegalEp transfer on the same AH grids.
Under the unified rebuild Soft~O2 also lifts exact replay (DISC $0.916$ vs FlatIP $0.714$; Lawyer $0.976$ vs $0.678$), but we still treat exact as diagnostic relative to paraphrase and advice-recall.
Paraphrase yields large Soft~O2 gains over FlatIP: $0.883$ vs $0.696$ on DISC and $0.956$ vs $0.697$ on Lawyer.
...
Soft~O2 recovers such siblings by propagating attenuated scores through session links, which helps precisely when the stored advice turn has weak direct lexical overlap with the query (paraphrase and advice-recall under the unified rebuild; Tables~\ref{tab:soft-o2-controls} and~\ref{tab:ndcg-graded}).
...
The gap-ratio analysis (Section~\ref{sec:beta-theory}) motivates keeping $\beta$ close to but below unity; the AH grid search then peaks in $\{0.9,0.95,0.98\}$ rather than at hard hydration ($\beta{=}1$).
On CAIL under the unified rebuild, Soft~O2 raises Answer Hit by large margins over FlatIP (Uk $0.932$ vs $0.432$; U-last $0.957$ vs $0.393$) with collapsing shuffled-$sid$ controls.
Hard hydration can exceed Soft~O2 AH/EC by unrestricted score copy.
After Holm adjustment on the Soft~O2 vs FlatIP family (Table~\ref{tab:sig}), the non-exact CAIL and LegalEp gains remain significant.
Under the unified rebuild, LegalEp exact channels also show Soft~O2 AH gains (Tables~\ref{tab:disc_main}--\ref{tab:lawyer_main}); paraphrase and advice-recall remain the main transfer evidence.
Graded nDCG agrees with the Soft~O2$>$FlatIP ordering (Table~\ref{tab:ndcg-graded}).
Under Mix, gated Hybrid significantly improves overall AH over Soft~O2 on LegalEp-DISC and LegalEp-Lawyer, while Soft~O2 remains strongest on LegalMem-MT (Table~\ref{tab:csce}); Soft~O2 stays preferable when gold already shares $sid$ (Table~\ref{tab:cluster_same}).
...
Soft~O2 is the main algorithmic contribution: attenuated session binding completes episodes without hard score copy, while hard hydration remains a stronger unrestricted score-copy upper bound on the unified rebuild grids.
Soft~O2-C extends the same idea to thematic clusters under Mix reconstructions as a conditional mechanism test.
Attenuation $\beta{=}0.98$ follows a three-candidate score-competition analysis together with an Answer Hit grid search over $\{0.5,0.7,0.9,0.95,0.98,1.0\}$; O1 FlatIP and O3 bulk-load are the implementation operators that make the shared-store ablations feasible.
On CAIL multi-turn and LegalEp paraphrase~/advice-recall channels, Soft~O2 improves Answer Hit and graded nDCG over FlatIP and cuts incompleteness; hard hydration can further raise AH/EC by unrestricted score copy, while shuffled-$sid$ controls collapse Soft~O2 gains.
