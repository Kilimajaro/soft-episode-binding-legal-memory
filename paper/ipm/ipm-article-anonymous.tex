...
The primary hypothesis family is Soft~O2 vs FlatIP on the nine CAIL and LegalEp transfer channels; for that family we report McNemar mid-$p$ on paired per-query Answer Hit and Holm-adjusted $p$-values (Table~\ref{tab:sig}), computed from a fixed paired-hit archive reserved for significance testing.
Appendix Soft~O2 grids report AH/EC/nDCG point estimates from the unified FlatIP rebuild without separate CI columns.
Hard-hydration, CE, and Soft~O2-C Mix contrasts are secondary.

Unless noted, Soft~O2 grids in Sections~\ref{sec:s1-strong-baselines}--\ref{sec:exp-main} and Appendix Tables~\ref{tab:cail_main}--\ref{tab:lawyer_main} use one FlatIP rebuild so that AH, EC, nDCG, and failure rates on each row come from the same per-query rankings (released metric JSON artifacts).
Holm $\Delta$AH values in Table~\ref{tab:sig} may differ slightly from rebuild point estimates because they use that paired-hit archive; directional Soft~O2$>$FlatIP conclusions on non-exact channels agree across both sources.
...
\begin{table}[H]
\centering
\caption{Primary Soft~O2 vs FlatIP family: McNemar mid-$p$ on paired Answer Hit, with Holm step-down adjustment ($m{=}9$). Values come from the fixed paired-hit archive released with the paper artifacts.}
\label{tab:sig}
\compacttab\footnotesize
\begin{tabular}{@{}llccc@{}}
\toprule
